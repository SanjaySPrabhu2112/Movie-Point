...
export const getMovies = (pageNumber) => {
    let selectedPageContents = moviesList.filter((item) => {
        return parseInt(item.page["page-num-requested"]) === pageNumber;
    })
    return new Promise((resolve, reject) => {
        if (!moviesList) {
            reject(new Error('Movies not found'));
        }
        if (selectedPageContents.length) {
            resolve(Object.values(selectedPageContents[0].page["content-items"].content));
        }
        else resolve([]);
    });
}
